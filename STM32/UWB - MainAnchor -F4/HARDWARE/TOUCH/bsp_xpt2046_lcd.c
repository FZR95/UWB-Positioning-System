...
uint8_t XPT2046_Touch_Calibrate(uint8_t LCD_Mode)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		LCD_ShowString(0, 0, lcddev.width, 16, 16, pStr);
	...
	LCD_ShowString(0, 20, lcddev.width, 16, 16, pStr);
...
Failure:
	...
	pStr = "try again";
	LCD_ShowString(0, 50, lcddev.width, 16, 16, pStr);
...
void XPT2046_TouchDown(strType_XPT2046_Coordinate *touch)
{
	...
	{
		int temp = POINT_COLOR;
		POINT_COLOR = RED;
		// LCD_Draw_Circle(touch->pre_x,touch->pre_y,4);
		POINT_COLOR = temp;
		// printf("Touch down : X=%d , Y=%d\n", touch->pre_x, touch->pre_y);
		UI_Input_Button_down(touch->pre_x, touch->pre_y);
	}
}
...
void XPT2046_TouchUp(strType_XPT2046_Coordinate *touch)
{
	...
	if (!(touch->pre_x == -1 && touch->pre_x == -1))
	{
		...
		UI_Input_Button_up(touch->pre_x, touch->pre_y);
	}
}
